//% weight=10 color=#0fbc11 icon="\uf108" block="gesture"
namespace gesture {

    export const initRegisterArray: number[] = [
        
    ];

    export enum EFGesture {
        //% block=None
        None = 0,
        //% block=Left
        Right = 1,
        //% block=Right
        Left = 2,
        //% block=Down
        Up = 3,
        //% block=Up
        Down = 4,
        //% block=Forward
        Forward = 5,
        //% block=Backward
        Backward = 6,
        //% block=Clockwise
        Clockwise = 7,
        //% block=Anticlockwise
        Anticlockwise = 8,
        //% block=Wave
        Wave = 9
    }

    export const gestureEventId = 3100;
    export let lastGesture = EFGesture.None;
    export let distanceBackup: number = 0;



    export function paj7620WriteReg(addr: number, cmd: number) {
        let buf: Buffer = pins.createBuffer(2);

        buf[0] = addr;
        buf[1] = cmd;

        pins.i2cWriteBuffer(0x73, buf, false);
    }

    export function paj7620ReadReg(addr: number): number {
        let buf: Buffer = pins.createBuffer(1);

        buf[0] = addr;

        pins.i2cWriteBuffer(0x73, buf, false);

        buf = pins.i2cReadBuffer(0x73, 1, false);

        return buf[0];
    }

    export function paj7620SelectBank(bank: number) {
        if (bank == 0) paj7620WriteReg(0xEF, 0);
        else if (bank == 1) paj7620WriteReg(0xEF, 1);
    }

    export function paj7620Init() {
        let temp = 0;

        paj7620SelectBank(0);

        temp = paj7620ReadReg(0);
        if (temp == 0x20) {
            for (let i = 0; i < 438; i += 2) {
                paj7620WriteReg(initRegisterArray[i], initRegisterArray[i + 1]);
            }
        }
        paj7620SelectBank(0);
    }

    //% blockId=gefesture_create_event block="on Gesture|%gesture"
    export function onGesture(gesture: EFGesture, handler: Action) {
        control.onEvent(gestureEventId, gesture, handler);
        init();
        control.inBackground(() => {
            while (true) {
                const gesture = read();
                if (gesture != lastGesture) {
                    lastGesture = gesture;
                    control.raiseEvent(gestureEventId, lastGesture);
                }
                basic.pause(50);
            }
        })

    }

    //% blockId=gesture_init block="initiate the Gesture"
    export function init(): void {
        paj7620Init();
        basic.pause(200);
    }

    //% blockId=gesture_read block="get gesture"
    export function read(): number {
        let data = 0, result = 0;
        data = paj7620ReadReg(0x43);
        switch (data) {
            case 0x01:
                result = EFGesture.Right;
                break;

            case 0x02:
                result = EFGesture.Left;
                break;

            case 0x04:
                result = EFGesture.Up;
                break;

            case 0x08:
                result = EFGesture.Down;
                break;

            case 0x10:
                result = EFGesture.Forward;
                break;

            case 0x20:
                result = EFGesture.Backward;
                break;

            case 0x40:
                result = EFGesture.Clockwise;
                break;

            case 0x80:
                result = EFGesture.Anticlockwise;
                break;

            default:
                data = paj7620ReadReg(0x44);
                if (data == 0x01)
                    result = EFGesture.Wave;
                break;
        }
        return result;
    }

}
